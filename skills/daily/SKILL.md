...
이월 항목이 있을 때만 `## 이월된 미완 항목` 섹션을 포함하고, 없으면 섹션 전체를 생략한다.
...
> 형식: `- [x] [[수정된 노트 위키링크]] 작업 내용 (PR #번호)`
> sub-bullet: 실제 변경 파일 경로, 핵심 수정 내용, 발견 사항
...
| 이전 데일리 노트 없음 | `## 이월된 미완 항목` 섹션 생략 |
| 이전 노트에 미완 항목 없음 | `## 이월된 미완 항목` 섹션 생략 |
