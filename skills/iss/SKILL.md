...
- 새 step/comms 작성 시, 또는 특정 ISS 갱신 요청 시 `ISS-{NNN} summary.md` 를 아래 **요약 노트 생성·갱신** 절차로 처리할 것 (없으면 생성, 있으면 갱신)

## 요약 노트 생성·갱신 (히스토리 축적)

`ISS-{NNN} summary.md` 는 **A안(축적)** 으로 운영한다 — 생성 시엔 배경 seed만 있고, steps/comms가 추가·갱신될 때마다 히스토리를 이어 붙인다. **특정 ISS를 지정해 갱신 요청**하면 summary.md가 **없으면 새로 작성(backfill), 있으면 갱신**한다 — 요약 노트 없이 발행된 **기존 ISS도 이 경로로 소급 생성**된다.

**트리거**: 사용자가 "ISS-{NNN} 갱신 / 요약 갱신" 등 특정 ISS를 지정해 요청 시, 또는 새 step/comms 작성 후.

**절차**:
1. **summary.md 존재 확인** — 없으면 위 `#### summary:` 템플릿 구조로 파일을 먼저 만든다(`created`·`updated` = 오늘). 있으면 그대로 2단계로.
2. 해당 ISS의 `steps/*.md` 전체 + `comms/*.md` 전체 + hub `## 진행 기록` 을 Read
3. 시간순(step 번호·comms 날짜)으로 정렬해 **`## 배경`(발생/발견 국면) · `## 경과`(서사)를 재구성** — 각 국면을 "무엇이 있었고 → 무엇을 확인했고 → 어디로 갔는지" 한 흐름으로 잇는다
4. **`## 현재 상태`** 를 최신 국면 기준으로 갱신 (진행 중이면 열린 스레드·다음 방향 명시)
5. frontmatter `updated:` 를 오늘 날짜로 갱신
...
- backfill(신규 생성) 시에도 tally가 아니라 **서사**로 — 이미 종료된 ISS면 `## 현재 상태`에 "종료 — {결말 요약}"으로 마무리
...
| 요약 노트 생성·갱신 | 특정 ISS 지정 갱신 요청 시: summary.md **없으면 backfill 생성, 있으면 갱신** (기존 ISS 소급 생성 포함). steps·comms·진행 기록 재구성, 완료/미완 tally 금지 |
