...
|---|------------------------|------|-------------------|
| 1 | {제목} | 없음 / `[[{기존 노트 파일명}]]`와 중복 | 챕터 내 핵심 문장 |
| 2 | ... | ... | ... |
...
3. **Fan-out (역링크 삽입)** — 새 노트와 관련 있는 기존 permanent 노트에 백링크 추가:
   ...
   3. 사용자가 승인한 후보의 `## 관련 메모` 섹션에 `- [[{새 노트 파일명}]]` 라인을 `Edit`으로 삽입합니다. 파일명과 H1이 다르면 `- [[{새 노트 파일명}|{제목}]]`로 써요
...
**링크 규칙**: 이 스킬이 쓰는 `[[ ]]`는 모두 [obsidian-markdown](../../references/obsidian-markdown.md)의 `링크 대상 규칙`(#8607 D6)을 따릅니다. 대상은 H1 제목이 아니라 실제 파일명(`.md` 제외)이에요. MOC처럼 파일명과 H1이 다르면 `[[tech-moc|Tech MOC]]`처럼 표시명을 붙이고, 맞는 MOC가 없으면 역링크 줄을 지웁니다. 챕터 파일(`ch01-...`)처럼 같은 파일명이 vault에 여럿이면 경로형으로 걸어요. 표 셀 안에서는 `\|`로 이스케이프하고, 없는 노트에는 링크를 걸지 않습니다.
